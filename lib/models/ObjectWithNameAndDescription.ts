...
export class ObjectWithNameAndDescription {
    ...
    constructor(right: {
        ...
        this.initObject(right)
    }

    public static generateSubject(name: string, description?: string, subject?: string): string {

        let result: string = "";
        if (subject) {
            result = subject;
        } else if (description) {
            result = description?.substring(0,
    ...
    protected initObject = (right: {
        ...
        this.subject = ObjectWithNameAndDescription.generateSubject(right.name, right.description, right.subject);
    }
}
